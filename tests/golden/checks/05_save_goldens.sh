#!/usr/bin/env bash
# Test 5: save golden non-empty, well-formed & stable.
#
...
# 3894:4 save timestamp l (game_save.rs (sv_write timestamp), wrLong(getCurrentUnixTime()))
# 3910:4 date_of_birth (game_save.rs (sv_write date_of_birth), wrLong(py.misc.date_of_birth))
# Runs the Rust umoria binary under a pty; likely needs required_permissions:["all"].
set -euo pipefail
