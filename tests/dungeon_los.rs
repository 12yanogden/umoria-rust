...
#[test]
fn look_see_floor_object_does_not_panic() {
    reset_for_new_game(None);
    umoria::ui_io::test_set_ncurses_stub(true);
    umoria::ui_io::test_clear_getch_keys();
    umoria::ui_io::test_push_getch_keys(&[i32::from(umoria::ui_io::ESCAPE)]);
    ...
    with_state_mut(|s| {
        let mut item = umoria::inventory::Inventory::default();
        umoria::inventory::inventory_item_copy_to(1, &mut item);
        s.game.treasure.list[1] = item;
        s.dg.floor[15][15].treasure_id = 1;
        s.dg.floor[15][15].temporary_light = true;
        s.dg.floor[15][15].field_mark = true;
    });

    los_look_reset_for_test();
    los_look_set_frame(0, 1, 1, 0);
    los_look_set_hack_no_query(false);
    los_look_set_rocks_and_objects(0);
