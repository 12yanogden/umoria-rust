...
pub fn cast_spell(spell_id: i32) {
    ...
    match spell_id {
        ...
        id if id == MageSpellId::StinkingCloud as i32
            && get_direction_with_memory(None, &mut dir) =>
        {
            spell_fire_ball(pos, dir, 12, MagicSpellFlags::PoisonGas, SPELL_NAMES[6]);
        }
        id if id == MageSpellId::Confusion as i32 && get_direction_with_memory(None, &mut dir) => {
        ...
        id if id == MageSpellId::Sleep1 as i32 && get_direction_with_memory(None, &mut dir) => {
        ...
        id if id == MageSpellId::FrostBolt as i32 && get_direction_with_memory(None, &mut dir) => {
            spell_fire_bolt(
                pos,
                dir,
        ...
        id if id == MageSpellId::WallToMud as i32 && get_direction_with_memory(None, &mut dir) => {
        ...
        id if id == MageSpellId::FireBolt as i32 && get_direction_with_memory(None, &mut dir) => {
            spell_fire_bolt(
                pos,
                dir,
        ...
        id if id == MageSpellId::FrostBall as i32 && get_direction_with_memory(None, &mut dir) => {
            spell_fire_ball(pos, dir, 48, MagicSpellFlags::Frost, SPELL_NAMES[24]);
        }
        ...
        id if id == MageSpellId::HasteSelf as i32 => {
            with_state_mut(|state| {
                ...
                state.py.flags.fast +=
                    (random_number_state(state, 20) + i32::from(state.py.misc.level)) as i16;
            });
        }
        id if id == MageSpellId::FireBall as i32 && get_direction_with_memory(None, &mut dir) => {
            spell_fire_ball(pos, dir, 72, MagicSpellFlags::Fire, SPELL_NAMES[28]);
        }
